...
// Rows
void printGridAndValues(int rows, int cols, int** board, char** operations, int** visited, int currentRow, int currentCol) {
...
void printBoard(int rows, int cols, int** board, char** operations, int** visited, const Player& p1, const Player& p2, int currentPlayerID) {
...
bool isValidMove(int newR, int newC, const Player& p,
    ...
    if (newR == other.row && newC == other.col) return false; //can not step on the other player's cells
    return true;
}
    ...
    int currentPlayerID)
{
    ...
    ofs.close();
    safeStrCopy(statusMsg, "Auto-saved successfully!", 256);
    cout << "[AUTO-SAVE] Updated \"" << filename;
    cout << "\" (currentPlayerID=" << currentPlayerID << ")\n";
}

bool tryPlayerMove(Player& currentP, Player& otherP, int rows, int cols, int** board, char** operations, int** visited, int& currentPlayerID) {
    while (true) {
...
bool loadGame(const char* filename, int& rows, int& cols, int**& board, char**& operations, int**& visited, Player& p1, Player& p2, int& currentPlayerID) {
...
void randomlyPlaceOperation(int** board, char** operations,
    int rows, int cols,
...
//The function that initializes random ops in the matrix
void generateMatrices(int** board, char** operations, int rows, int cols) {
    ...
    //fill remaining cells with random ops
...
int main() {
    ...
    if (choice == NEW_GAME) {
        ...
        srand((unsigned)time(nullptr)); //to generate random values
    ...
    while (!gameOver) {
        ...
        while (!validMove) {
            ...
            if (!validMove) {
                ...
                printBoard(rows, cols, board, operations, visited, p1,
                    p2, currentPlayerID);
            }
        }
        if (gameOver) break;
